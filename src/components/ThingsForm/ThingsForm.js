...
import { validateThings } from '../../utility/validator/formValidation/formValidation';
...
const ThingsForm = (props) => {
    ...
    const addCategoryId = (Things, categories) => {
        const currentLocation = props.history.location.pathname;
        categories.forEach(categorie => {
            if (categorie.name === Categories.THINGS) {
                Things['category_id'] = categorie.id;
            }
        })
    }

    const addOrUpdateThingsService = (values) => {
        const Things = cloneDeep(values);
        if (!Things.address.zip || Things.address.zip === '') {
            delete Things.address;
        } else {
            removeEmptyStringKeys(Things.address);
        }

        const a = Things.compensation_ids;
        const set = new Set([a]);
        const b = Array.from(set);
        Things.compensation_ids = b;


        if (props.profilePicPath) {
            Things.profilePicPath = props.profilePicPath
        }


        // Things.compensation_ids = values.compensation_ids && values.compensation_ids.map(c => {
        //     return c.value
        // })
        // Things.address.zip = Things.address.zip.toString();
        // Things['profile_id'] = props.user.primary_profile_id;
        addCategoryId(Things, props.categories);
        if (props.defaultImagePath) {
            Things.default_image_path = props.defaultImagePath
        }
        if (props.defaultBase64Index) {
            Things.defaultBase64Index = props.defaultBase64Index
        }
        props.addOrUpdateThingsService(Things);
    ...
    return (
        <Formik
            ...
            validate={(values) => validateThings(values, setAddressDisable)}
            onSubmit={addOrUpdateThingsService}
        >
            {(formicProps) => {
                ...
                return (
                    <Form className="tab-pane fade show active" id="pills-things" role="tabpanel" aria-labelledby="pills-things-tab">
                        {!isSafari() && <Prompt
                        ...
                        <div className="card switch_card_list card_bg_300">
                            <div className="card-body">
                                ...
                                <div className="form_group_modify">
                                    <label for="Description" className="label_modify">Description/Type/ Timeframe <span className="text-danger">*</span></label>
                                    <Field name="description" as="textarea" id="ServiceDescription" className="input_modify input_modify_lg" rows="4"
                                        ...
                                        placeholder={`Description: Enter a complete description of the item you have available, including the price, and keywords others will use to find it.\nType: Also, what category does the thing you're offering fall within (Video equipment, vehicles, books, Office furniture, etc.)?\nTime Frame: Finally, when is it available and for how long?  Availble right now? `}
                                    />
                                ...
                                <div className="actions_btns w-100 text-right">
                                    <button className="theme_btn theme_btn_gray text-uppercase" type="button" onClick={props.cancelThingsClicked}>Cancel</button>
