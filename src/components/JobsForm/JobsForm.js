...
import { validateJobsOrGigs } from '../../utility/validator/formValidation/formValidation';
...
const JobsForms = (props) => {
    ...
    const addCategoryId = (JobsOrGigs, categories) => {
        const currentLocation = props.history.location.pathname;
        categories.forEach(categorie => {
            if (categorie.name === Categories.JOBS_GIGS) {
                JobsOrGigs['category_id'] = categorie.id;
            }
        })
    }

    const addOrUpdateJobsOrGigs = (values) => {
        const JobsOrGigs = cloneDeep(values);

        if (!JobsOrGigs.address && !JobsOrGigs.address.zip || JobsOrGigs.address.zip === '') {
            delete JobsOrGigs.address;
        } else {
            removeEmptyStringKeys(JobsOrGigs.address);
        }

        JobsOrGigs.skills = values.skills && values.skills.map(skill => {
            return {
                id: skill.value,
                name: skill.label
            }
        ...
        const a = JobsOrGigs.compensation_ids;
        const set = new Set([a]);
        const b = Array.from(set);
        JobsOrGigs.compensation_ids = b;

        if (props.profilePicPath) {
            JobsOrGigs.profilePicPath = props.profilePicPath
        }
        // JobsOrGigs.compensation_ids = values.compensation_ids && values.compensation_ids.map(c => {
        //     return c.value
        // })

        // JobsOrGigs.address.zip = JobsOrGigs.address.zip.toString();
        // JobsOrGigs['profile_id'] = props.user.primary_profile_id;
        addCategoryId(JobsOrGigs, props.categories);
        if (props.defaultImagePath) {
            JobsOrGigs.default_image_path = props.defaultImagePath
        }
        if (props.defaultBase64Index) {
            JobsOrGigs.defaultBase64Index = props.defaultBase64Index
        }
        props.addOrUpdateJobsOrGigs(JobsOrGigs);
    ...
    return (
        <Formik
            ...
            validate={(values) => validateJobsOrGigs(values, setAddressDisable)}
            onSubmit={addOrUpdateJobsOrGigs}
        >
            {(formicProps) => {
                ...
                return (
                    <Form className="tab-pane fade show active" id="pills-jobs" role="tabpanel" aria-labelledby="pills-jobs-tab">
                        {!isSafari() && <Prompt
                                ...
                                <div className="card-body">
                                    <div className="ph_title_btn ph_flex_wrp_spw ph_xs_offer_reverse">
                                        ...
                                        {/* <a href="javascript:void(0)" id="create_new_offer" 
                                            ...
                                            onClick={props.createJobsOrGigs}
                                            >
                                                Create new offer
                                        </a> */}
                                    ...
                                    <div className="form_group_modify">
                                        <label for="ProjectDescription" className="label_modify">Description/Timeframe <span className="text-danger">*</span></label>
                                        <Field name="description" as="textarea" id="ServiceDescription" className="input_modify input_modify_lg" rows="4"
                                            ...
                                            placeholder={`Description: Enter a complete description of the work have available, the job you're trying to fill, along with keywords others will use to find it.\nType: Also, what category does the job or gig you're offering fall within (Video project, accounting, marketing campaign, voice-over needed etc.)?\nTime Frame: Finally, when do you need them and for how long?  Do you need somebody right now? `}
                                        />
                                    ...
                                    <div className="form_group_modify">
                                        ...
                                        <Field name="cool_feature" type="text" className="input_modify input_modify_lg" id="WhyCool" placeholder="What makes this job/gig challenging, fun or interesting?" />
                                    </div>
                                    <div className="form_group_modify">
                                    ...
                                    <div className="form-row">
                                        ...
                                        <div className="form_group_modify col-md-4">
                                            ...
                                            <Field name="specification" type="text" className="input_modify input_modify_lg" id="Education" placeholder="List your credentials, experience, or relevant educational background." />
                                        </div>
                                    </div>
                                    {/* <div className="form-row">
                                    ...
                                    <div className="form-row">
                                        ...
                                        <div class="form_group_modify col-sm-4">
                                    ...
                                    <div className="actions_btns w-100 text-right">
                                        <button className="theme_btn theme_btn_gray text-uppercase" onClick={props.cancelJobsOrGigsClicked} type="button">Cancel</button>
