...
import { validateOpportunities } from '../../utility/validator/formValidation/formValidation';
...
import { removeEmptyStringKeys, getEditCompensations, isSafari, toastMsg } from '../../utility/utility';
import MultiSelectCheckBox from '../UI/MultiselectCheckBox/MultiselectCheckBox';
import CustomToolTip from '../UI/CustomToolTip/CustomToolTip';
import { Prompt, useHistory } from 'react-router-dom';
const cloneDeep = require('clone-deep');

const OpportunitesForm = (props) => {
    ...
    const addCategoryId = (Opportunities, categories) => {
        const currentLocation = props.history.location.pathname;
        categories.forEach(categorie => {
            if (categorie.name === Categories.SHARING_OPPORTUNITIES) {
                Opportunities['category_id'] = categorie.id;
            }
        })
    }

    const addOrUpdateOpportunities = (values) => {
        const Opportunities = cloneDeep(values);

        if (!Opportunities.address && !Opportunities.address.zip || Opportunities.address.zip === '') {
            delete Opportunities.address;
        } else {
            removeEmptyStringKeys(Opportunities.address);
        }

        addCategoryId(Opportunities, props.categories);
        // Opportunities.address.zip = Opportunities.address.zip.toString();

        const a = Opportunities.compensation_ids;
        const set = new Set([a]);
        const b = Array.from(set);
        Opportunities.compensation_ids = b;

        if (props.profilePicPath) {
            Opportunities.profilePicPath = props.profilePicPath
        }

        // Opportunities.compensation_ids = values.compensation_ids && values.compensation_ids.map(c => {
        //     return c.value
        // })
        Opportunities['profile_id'] = props.user.primary_profile_id;
        if (props.defaultImagePath) {
            Opportunities.default_image_path = props.defaultImagePath
        }
        if (props.defaultBase64Index) {
            Opportunities.defaultBase64Index = props.defaultBase64Index
        }
        props.addOrUpdateOpportunities(Opportunities);
    }
    ...
    return (
        <Formik
            ...
            validate={(values) => validateOpportunities(values, setAddressDisable)}
            onSubmit={addOrUpdateOpportunities}
        >
            {(formicProps) => {
                ...
                return (
                    <Form className="tab-pane fade show active" id="pills-sharing-opportunities" role="tabpanel" aria-labelledby="pills-sharing-opportunities-tab">
                        {!isSafari() && <Prompt
                            ...
                            <div className="card-body">
                                ...
                                <div className="form_group_modify">
                                    ...
                                    <Field name='title' type="text" className="input_modify input_modify_lg" id="Title"
                                        ...
                                        placeholder="Enter the headline or title of what you're willing to share"
                                    />
                                    <ErrorMessage name="title" render={msg => <span style={formInputTextErrorStyle}>{msg}</span>} />
                                </div>
                                <div className="form_group_modify">
                                    <label for="Description" className="label_modify">Description/Timeframe/Sharing Proposal <span className="text-danger">*</span></label>
                                    <Field name="description" as="textarea" id="ServiceDescription" className="input_modify input_modify_lg" rows="4"
                                        ...
                                        placeholder={`Description: Enter a complete description of what have available to share with others, how you will share costs, along with keywords others will use to find it.\nType: Also, what category does your shared item fall within (Vehicle, Office Space, Software subscription, Email list, Lab equipment, etc.)?\nTime Frame: Finally, when is this available and for how long?  What is your proposed sharing arangement?`}
                                    />
                                ...
                                <div className="form_group_modify">
                                    ...
                                    <Field name="cool_feature" type="text" className="input_modify input_modify_lg" id="WhyCool" placeholder="What makes this sharing opportunity unique and irresistable?" />
                                </div>
                                <div className="form_group_modify">
                                    <label for="Specifications" className="label_modify">Specifications</label>
                                    <Field name="specification" as="textarea" className="input_modify" id="Specifications" rows="4" placeholder="This is a space to list detailed specifications, dimensions, or any other details you choose." />
                                </div>
                                ...
                                <div className="form-row">
                                    <div class="form_group_modify col-sm-6">
                                        <label for="Compensation" className="label_modify">Posted by {/*(Orange color denotes all the business profiles)*/}: <span className="text-danger">*</span></label>
                                ...
                                <div className="actions_btns w-100 text-right">
                                    <button className="theme_btn theme_btn_gray text-uppercase" type="button" onClick={props.cancelOpportunitiesClicked}>Cancel</button>
...
export default OpportunitesForm;