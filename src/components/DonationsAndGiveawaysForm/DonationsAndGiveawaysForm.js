...
import { validateDonationsOrGiveaways } from '../../utility/validator/formValidation/formValidation';
...
const DonationsAndGiveawaysForm = (props) => {
    ...
    const setInitialValues = {
        ...
        compensation_ids: props.service ? getEditCompensations(props.service.compensations) : props.compensations[0].id,
    ...
    const addCategoryId = (Donations, categories) => {
        const currentLocation = props.history.location.pathname;
        categories.forEach(categorie => {
            if (categorie.name === Categories.DONATIONS ) {
                Donations['category_id'] = categorie.id;
            }
        })
    }

    const addOrUpdateDonations = (values) => {
        const Donations = cloneDeep(values);

        if (!Donations.address && !Donations.address.zip || Donations.address.zip === '') {
            delete Donations.address;
        } else {
            removeEmptyStringKeys(Donations.address);
        }

        addCategoryId(Donations, props.categories);
        // Donations.address.zip = Donations.address.zip.toString();

        const a = Donations.compensation_ids;
        const set = new Set([a]);
        const b = Array.from(set);
        Donations.compensation_ids = b;

        if (props.profilePicPath) {
            Donations.profilePicPath = props.profilePicPath
        }

        // Donations.compensation_ids = values.compensation_ids && values.compensation_ids.map(c => {
        //     return c.value
        // })

        Donations['profile_id'] = props.user.primary_profile_id;
        if (props.defaultImagePath) {
            Donations.default_image_path = props.defaultImagePath
        }
        if (props.defaultBase64Index) {
            Donations.defaultBase64Index = props.defaultBase64Index
        }
        props.addOrUpdateDonations(Donations);
        props.selectedCategory('People')
    }
    ...
    return (
        <Formik
            ...
            validate={(values) => validateDonationsOrGiveaways(values, setAddressDisable)}
            onSubmit={addOrUpdateDonations}
        >
            {(formicProps) => {
                ...
                return (
                    <Form className="tab-pane fade show active" id="pills-donations-giveaways" role="tabpanel" aria-labelledby="pills-donations-giveaways-tab">
                        {!isSafari() && <Prompt
                            ...
                            <div className="card-body">
                                ...
                                <div className="form_group_modify">
                                    ...
                                    <Field name='title' type="text" className="input_modify input_modify_lg" id="Title"
                                        ...
                                        placeholder={'Enter the headline or title of what you want to give for free'}
                                    />
                                    <ErrorMessage name="title" render={msg => <span style={formInputTextErrorStyle}>{msg}</span>} />
                                </div>
                                <div className="form_group_modify">
                                    <label for="Description" className="label_modify">Description <span className="text-danger">*</span></label>
                                    <Field name="description" as="textarea" id="ServiceDescription" className="input_modify input_modify_lg" rows="4"
                                        ...
                                        placeholder={`Description: Enter a complete description of what you're giving away, and keywords others will use to find it.\nType: Also, what category does your giveaway fall within (Furniture, courses, eBook, consultation)?\nTime Frame: Finally, when is this available and for how long?`}
                                    />
                                ...
                                <div className="form_group_modify">
                                    ...
                                    <Field name="cool_feature" type="text" className="input_modify input_modify_lg" id="WhyCool" placeholder="What makes this giveaway unique and irresistable?" />
                                </div>
                                <div className="form_group_modify">
                                ...
                                <div className="form-row">
                                    <div className="form_group_modify col-md-3">
                                        ...
                                        <Field as="select" name='categories' className="input_modify cstSelect"
                                    ...
                                    <div className="form_group_modify col-md-3">
                                        <label for="Compensation" className="label_modify">Compensation </label>
                                        {/* <MultiSelectCheckBox
                                    ...
                                    <label className="label_modify">Upload images <span className="text-danger">*</span> -  <i style={{ fontWeight: 400 }}>(Maximum 4 images) Please click on the image to mark it as default image for an offer. Default image will be shown in searches. Upload at least 1 offer image to successfully post this offer. </i>
                                        {
                                            props.profilePicPath
                                                ? null
                                                : (props.service && props.service.attachments && props.service.attachments.length > 0) || (props.base64.length > 0)
                                                    ? null
                                ...
                                <div className="actions_btns w-100 text-right">
                                    <button className="theme_btn theme_btn_gray text-uppercase" type="button" onClick={props.cancelDonationsClicked}>Cancel</button>
...
export default DonationsAndGiveawaysForm;