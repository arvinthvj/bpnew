...
import ThingsForm from "../../../components/ThingsForm/ThingsForm";
...
class Things extends Component {
  constructor(props) {
    ...
    // this.editThingsClicked = this.editThingsClicked.bind(this);
    // this.cancelThingsClicked = this.cancelThingsClicked.bind(this);
  }
  ...
  componentDidMount = () => {
    ...
    if (isEdit && service) {
      this.editThingsClicked(storage.get("service", null));
      this.props.getThingsServiceList(
  ...
  addOrUpdateThingsService = (Things) => {
    ...
    if (
      ...
      if (Things.address) {
        delete Things.address.id;
        delete Things.address.state_code;
      }
      Things["delete_attachment_ids"] = this.state.idsToDelete;
      ...
      if (this.state.base64 && this.state.base64.length > 0) {
        ...
        this.props.updateThingsServiceWithImage(
          this.state.service.id,
          Things,
          extentions,
          arrayBuffer
        );
      } else if (Things.profilePicPath) {
        let photo_paths = [];
        photo_paths.push(Things.profilePicPath);
        Things["new_photo_paths"] = photo_paths;
        delete Things.profilePicPath;
        this.props.updateThingsService(this.state.service.id, Things);
      } else {
        this.props.updateThingsService(this.state.service.id, Things);
      }
    } else {
      if (this.state.base64 && this.state.base64.length > 0) {
        ...
        const extentions = extractExtention(this.state.imageName);
        this.props.addThingsServiceWithImages(Things, extentions, arrayBuffer);
      } else if (Things.profilePicPath) {
        let photo_paths = [];
        photo_paths.push(Things.profilePicPath);
        Things["photo_paths"] = photo_paths;
        delete Things.profilePicPath;
        this.props.addThingsService(Things);
      } else {
        // this.props.addThingsService(Things);
            ...
            <div className="ph_flex_wrp_spw ph_switch_wrp align-items-start">
              ...
              <div className="ph_switch_con ph_crud_wrp">
                ...
                <a
                  ...
                  onClick={() => this.editThingsClicked(service)}
                >
      ...
      if (
          ...
          >
            ...
            >
              ...
              <div class="text-center mt-2">
                <a
                  href="javascript:void(0)"
                  onClick={this.createThingsClicked}
                  className="theme_btn theme_primary"
                >
                  CREATE NEW OFFER
                </a>
        ...
        return (
          <div
            ...
            aria-labelledby="pills-my-services-tab"
          >
            {this.props.isFormOpen || this.state.showThingsForm ? (
              <ThingsForm
                {...this.state}
                {...this.props}
                ...
                addOrUpdateThingsService={this.addOrUpdateThingsService}
                cancelThingsClicked={this.cancelThingsClicked}
              />
            ) : (
...
const mapDispatchToProps = (dispatch) => {
  return {
    getThingsServiceList: (profileId, categorieId) =>
      dispatch(actions.getServicesList(profileId, categorieId)),
    addThingsService: (service) => dispatch(actions.addService(service)),
    addThingsServiceWithImages: (service, extentions, arrayBuffer) =>
      dispatch(actions.addServiceWithImages(service, extentions, arrayBuffer)),
    updateThingsService: (id, service) =>
      dispatch(actions.updateService(id, service)),
    updateThingsServiceWithImage: (id, service, extentions, arrayBuffer) =>
      dispatch(
