...
class Places extends Component {
  constructor(props) {
    ...
    // this.addPlaceService = this.addPlaceService.bind(this);
    // this.editPlaceClicked = this.editPlaceClicked.bind(this);
    // this.cancelPlaceClicked = this.cancelPlaceClicked.bind(this);
  }
  ...
  componentDidMount = () => {
    ...
    if (isEdit && service) {
      this.editPlaceClicked(storage.get("service", null));
      this.props.getPlacesServiceList(
  ...
  addOrUpdatePlaceService = (Place) => {
    ...
    if (
      ...
      if (Place.address) {
        delete Place.address.id;
        delete Place.address.state_code;
      }
      Place["delete_attachment_ids"] = this.state.idsToDelete;
      ...
      if (this.state.base64 && this.state.base64.length > 0) {
        ...
        this.props.updatePlaceServiceWithImage(
          this.state.service.id,
          Place,
          extentions,
          arrayBuffer
        );
      } else if (Place.profilePicPath) {
        let photo_paths = [];
        photo_paths.push(Place.profilePicPath);
        Place["new_photo_paths"] = photo_paths;
        delete Place.profilePicPath;
        this.props.updatePlaceService(this.state.service.id, Place);
      } else {
        this.props.updatePlaceService(this.state.service.id, Place);
      }
    } else {
      if (this.state.base64 && this.state.base64.length > 0) {
        ...
        const extentions = extractExtention(this.state.imageName);
        this.props.addPlaceServiceWithImages(Place, extentions, arrayBuffer);
      } else if (Place.profilePicPath) {
        let photo_paths = [];
        photo_paths.push(Place.profilePicPath);
        Place["photo_paths"] = photo_paths;
        delete Place.profilePicPath;
        this.props.addPlaceService(Place);
      } else {
        // this.props.addPlaceService(Place);
            ...
            <div className="ph_flex_wrp_spw ph_switch_wrp align-items-start">
              ...
              <div className="ph_switch_con ph_crud_wrp">
                ...
                <a
                  ...
                  onClick={() => this.editPlaceClicked(service)}
                >
      ...
      if (
          ...
          >
            ...
            >
              ...
              <div class="text-center mt-2">
                <a
                  href="javascript:void(0)"
                  onClick={this.createPlaceClicked}
                  className="theme_btn theme_primary"
                >
                  CREATE NEW OFFER
                </a>
        ...
        return (
          <div
            ...
            aria-labelledby="pills-my-services-tab"
          >
            {this.props.isFormOpen || this.state.showPlace ? (
              <PlaceForm
                {...this.state}
                {...this.props}
                createPlaceClicked = {this.createPlaceClicked}
                ...
                addOrUpdatePlaceService={this.addOrUpdatePlaceService}
                cancelPlaceClicked={this.cancelPlaceClicked}
              />
            ) : (
...
const mapDispatchToProps = (dispatch) => {
  return {
    getPlacesServiceList: (profileId, categorieId) =>
      dispatch(actions.getServicesList(profileId, categorieId)),
    addPlaceService: (service) => dispatch(actions.addService(service)),
    addPlaceServiceWithImages: (service, extentions, arrayBuffer) =>
      dispatch(actions.addServiceWithImages(service, extentions, arrayBuffer)),
    updatePlaceService: (id, service) =>
      dispatch(actions.updateService(id, service)),
    updatePlaceServiceWithImage: (id, service, extentions, arrayBuffer) =>
      dispatch(
